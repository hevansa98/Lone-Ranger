...
namespace nmea
{
    ...
    private:
        ...
        std::shared_ptr<std::binary_semaphore> Satellite_Table_Semaphore;
    ...
    void nmea_format_gsv::Parse(const std::string & RAW_Input)
    {
        ...
        {
            Currently_Processed_Message_Dbg[Field] = "";
        }

        for(std::string::iterator Current_Character = RAW_String.begin(); Current_Character != RAW_String.end(); Current_Character++)
        {
            ...
            if((Currently_Processed_Field == FIELDS::SATELLITES_IN_VIEW) && (*Current_Character == ','))
            {
                ...
                if(Current_Message == STARTING_GSV_BUNDLE_MESSAGE_ID)
                {
                    ...
                    //std::cout << "Number of sats: " << Satellite_Table->capacity() << " " << stoul(Currently_Processed_Message_Dbg[(size_t)FIELDS::SATELLITES_IN_VIEW]) << std::endl;
                }
            }
    ...
    void nmea_format_gsv::Assign_Satellite_Table_Mutex(std::vector<Satellite_Data_Type> * Satellite_Table_p, std::shared_ptr<std::binary_semaphore> Satellite_Semaphore)
    {
        this->Satellite_Table = Satellite_Table_p;

        if(Satellite_Semaphore != nullptr)
        {
            this->Satellite_Table_Semaphore = Satellite_Semaphore;
            std::cout << Satellite_Table_Semaphore.use_count() << std::endl;
        }
    }
